...
options_templates.update(options_section(('sampler-params', "Sampler parameters"), {
    ...
    "dpm_adapt_start": OptionHTML("For dpm adaptive"),
    'rtol': OptionInfo(default=0.05, label="relative tolerance", component=gr.Slider, component_args={"minimum": 0.0, "maximum": 1.0, "step": 0.001}, infotext='Relative tolerance').info('for dpm adaptive, determines how much the error can deviate from the desired accuracy relative to the magnitude of the solution.'),
    "atol": OptionInfo(default=0.0078, label="absolute tolerance", component=gr.Slider, component_args={"minimum": 0.0, "maximum": 1.0, "step": 0.0001}, infotext='Absolute tolerance').info('determines how much the error can deviate from the desired accuracy regardless of the magnitude of the solution.'),
    "h_init": OptionInfo(default=0.05, label="initial step size", component=gr.Slider, component_args={"minimum": -1.0, "maximum": 1.0, "step": 0.01}, infotext='Initial step size').info('affects the starting point of the adaptive step size algorithm.'),
    "pcoeff": OptionInfo(default=0.0, label="proportional coefficient", component=gr.Slider, component_args={"minimum": 0.0, "maximum": 10.0, "step": 0.1}, infotext='Proportional coefficient').info('controls how much the step size changes based on the current error.'),
    "icoeff": OptionInfo(default=1.0, label="integral coefficient", component=gr.Slider, component_args={"minimum": 0.0, "maximum": 10.0, "step": 0.1}, infotext='Integral coefficient').info('controls how much the step size changes based on the accumulated error.'),
    "dcoeff": OptionInfo(default=0.0, label="derivative coefficient", component=gr.Slider, component_args={"minimum": 0.0, "maximum": 10.0, "step": 0.1}, infotext='Derivative coefficient').info('controls how much the step size changes based on the rate of change of the error.'),
    "eta": OptionInfo(default=0.0, label="reverse sampling parameter", component=gr.Slider, component_args={"minimum": 0.0, "maximum": 1.0, "step": 0.01}, infotext='Reverse sampling parameter').info('determines how much the solver samples from the past instead of the future when solving the ODE.'),
